...
SwaggerUi.partials.signature = (function () {
    ...
    var getModelSignature = function (name, schema, models, modelPropertyMacro) {
            ...
            if (schema.$ref) {
                ...
                if (_.isArray(schema.items)) {
                    ...
                    if (_.isUndefined(schema.items.$ref)) {
                        if (!_.isUndefined(schema.items.type) && _.indexOf(['array', 'object'], schema.items.type)
                                                                 === -1) {
        ...
        function primitiveToOptionsHTML(schema, html) {
            ...
            if (options.length > 0) {
                html =
                    '<span class="propWrap">' + html + '<table class="optionsWrapper"><tr><th colspan="2">' + type
                    + '</th></tr>' + options + '</table></span>';
            }
                ...
                if (_.isArray(schema.items)) {
                    ...
                    if (_.isUndefined(schema.items.$ref)) {
                        if (_.indexOf(['array', 'object'], schema.items.type || 'object') > -1) {
                            if ((_.isUndefined(schema.items.type) || schema.items.type === 'object') && _.isUndefined(
                    ...
                    if (_.isPlainObject(schema.properties)) {
                        contents = _.map(schema.properties, function (property, name) {
                            ...
                            if (!_.isUndefined(cProperty.$ref)) {
                                ...
                                if (!_.isUndefined(model) && _.indexOf([undefined, 'array', 'object'],
                                                                       model.definition.type) === -1) {
                            ...
                            return '<div' + (property.readOnly ? ' class="readOnly"' : '') + '>'
    ...
    // Commenting this funtion as the names are now determined beforehand and the prefix part is exposed as a separate
    ...
    var createArrayXML = function (descriptor) {
        ...
        var key = name;
        // If there is a name specified for the array elements, use that for the array elements name |
        // https://github.com/swagger-api/swagger-ui/issues/2577
    ...
    function Descriptor(name, type, definition, models, config) {
        ...
        // name is already set by getDescriptorByRef or getDescriptor function depending on the type. Only prefix, if
        // present is needed to be set here | https://github.com/swagger-api/swagger-ui/issues/2577
    ...
    function getDescriptorByRef($ref, name, models, config) {
        ...
        // If model definition xml name is present, then that will be preferred over model name. This is the case of
        // preferring XmlElement name over XmlRootElement name if XmlElement name is provided |
        // https://github.com/swagger-api/swagger-ui/issues/2577
        if (model.definition && model.definition.xml && model.definition.xml.name) {
            name = name || model.definition.xml.name || model.name;
        }
        // else only model name will be considered for determination |
        // https://github.com/swagger-api/swagger-ui/issues/2577
        else {
    ...
    function getDescriptor(name, definition, models, config) {
        ...
        // If definition xml name is present, then that will be preferred over name |
        // https://github.com/swagger-api/swagger-ui/issues/2577
