...
SwaggerUi.Views.AuthView = Backbone.View.extend({
                                                    ...
                                                    initialize: function (opts) {
                                                        ...
                                                        this.authsCollectionView =
                                                            new SwaggerUi.Views.AuthsCollectionView({data: opts.data});
                                                    ...
                                                    authorize: function () {
                                                        this.authsCollectionView.collection.forEach(function (auth) {
                                                            ...
                                                            if (type === 'apiKey') {
                                                                ...
                                                                this.router.api.clientAuthorizations.add(
                                                                    auth.get('title'), keyAuth);
                                                            } else if (type === 'basic') {
                                                                basicAuth =
                                                                    new SwaggerClient.PasswordAuthorization(
                                                                        auth.get('username'), auth.get('password'));
                                                                this.router.api.clientAuthorizations.add(
                                                                    auth.get('title'), basicAuth);
                                                            } else if (type === 'oauth2') {
                                                    ...
                                                    logoutClick: function (e) {
                                                        ...
                                                        this.authsCollectionView.collection.forEach(function (auth) {
                                                            window.swaggerUi.api.clientAuthorizations.remove(
                                                                auth.get('title'));
                                                        });
                                                    ...
                                                    handleOauth2Login: function (auth) {
                                                        ...
                                                        var pathname = location.pathname.substring(0,
                                                                                                   location.pathname.lastIndexOf(
                                                                                                       '/'));
                                                        var defaultRedirectUrl = host.protocol + '//' + host.host
                                                                                 + pathname + '/o2c.html';
                                                        ...
                                                        /**
                                                         * Returns the name of the access token parameter returned by
                                                         * the server.
                                                         *
                                                         * @param dets
                                                        ...
                                                        function getTokenName(dets) {
                                                            return dets.vendorExtensions['x-tokenName']
                                                                   || dets.tokenName;
                                                        }

                                                        if (auth.get('type') === 'oauth2' && flow && (flow
                                                                                                      === 'implicit'
                                                                                                      || flow
                                                                                                      === 'accessCode')) {
                                                            dets = auth.attributes;
                                                            url =
                                                                dets.authorizationUrl + '?response_type=' + (flow
                                                                                                             === 'implicit'
                                                                ? 'token' : 'code');
                                                        ...
                                                        else if (auth.get('type') === 'oauth2' && flow && (flow
                                                                                                           === 'application')) {
                                                            dets = auth.attributes;
                                                            container.tokenName = getTokenName(dets) || 'access_token';
                                                            this.clientCredentialsFlow(scopes, dets,
                                                                                       container.OAuthSchemeKey);
                                                            return;
                                                        }
                                                        else if (auth.get('type') === 'oauth2' && flow && (flow
                                                                                                           === 'password')) {
                                                            ...
                                                            for (var t in o) {
                                                                if (o.hasOwnProperty(t) && t === 'implicit') {
                                                                    ...
                                                                    url =
                                                                        dets.loginEndpoint.url + '?response_type=token';
                                                    ...
                                                    accessTokenRequest: function (scopes, oauth, OAuthSchemeKey,
                                                                                  grantType, params) {
                                                        params = $.extend({}, {
                                                            'scope': scopes.join(' '),
